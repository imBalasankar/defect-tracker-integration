...
export default function ManageDefectStatus() {
  ...
  useEffect(() => {
    Axios.get("http://localhost:8087/api/v1/status")
      .then(response => {
        ...
        setStatus(response.data.results.listAllStatus);
  ...
  const options = {
    ...
    textLabels: {
      body: {
        noMatch: status.length > 0 ? "Loading data..." : "No Records Found!"
      }
    },
    customToolbar: () => {
      return <DefectStatusCustomToolbar onCreate={handleTrackAdd} />;
    },
    customToolbarSelect: () => {
      return (
        <DefectStatusCustomToolbarSelect
    ...
    onRowsSelect: allRows => {
      allRows.forEach(row => {
        const dataRow = status[row.dataIndex];
  ...
  const handleDelete = () => {
    Axios.delete(`http://localhost:8087/api/v1/status/${values.id}`)
      .then(response => {
        console.log(response);
  ...
  return (
    <div>
      ...
      <Container className={classes.container}>
        ...
        <MUIDataTable data={status} columns={columns} options={options} />
      </Container>
    </div>
  );
}
